...
async def insert_tag(request: Request):
    try:
        ...
        for genre in movie_genres:
            ...
            existing_data = supabase.table("Users_Table").select("*").eq("UserId", user_id).eq("UserTags", genre).execute()
            ...
            if existing_data.data:
                ...
                # Users 테이블 업데이트
                update_response = supabase.table("Users_Table").update({"UserStack": user_stack}).eq("UserId", user_id).eq("UserTags", genre).execute()

            else:
                # 같은 사용자 ID와 영화 장르가 없으면 새로운 데이터 삽입
                insert_response = supabase.table("Users_Table").insert({
                    "UserId": user_id,
                    "UserTags": genre,
                    "UserStack": 1
                }).execute()
